...
const _screen = function (_node, _width, _height) {
  return {
    ...
    drawLetter: function (letter, letterRow, letterCol, shouldErase, fontSize) {
      ...
      const letterWidth = fontSize === 1 ? 8 : 16
      let letterBits = font[letter.charCodeAt(0)]
      let row = 0
      let col = 0
      while (letterBits !== 0) {
        // if firstBit is set draw it
        if (letterBits & 1) {
          if (bitPattern[bitPatternRow].charAt(bitPatternCol) === "1") {
            if (shouldErase) {
              this.resetPixel(row + letterRow * rowSpacer + 1, col + letterCol * colSpacer)
            } else {
              this.setPixel(row + letterRow * rowSpacer + 1, col + letterCol * colSpacer)
            }
          }
        }
        // shift it right
        letterBits = letterBits >>> 1
    ...
    drawLetter1: function (letter, letterRow, letterCol, shouldErase, fontSize) {
      fontSize = fontSize ? fontSize : 1
      const font = fontSize === 1 ? font1 : font2
      const rowSpacer = fontSize === 1 ? 10 : 20
      const colSpacer = fontSize === 1 ? 6 : 12
